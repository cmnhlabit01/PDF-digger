...
def clean_thai_ocr_text(text: str) -> str:
    ...
    # 1. แก้สระที่สลับตำแหน่งบ่อยๆ เช่น เกบ็ -> เก็บ, ไวเ้ -> ไว้, กวา่ -> กว่า, ไมต่อง้/ตอ้ง -> ต้อง
    ...
    text = re.sub(r'([ก-ฮ])([งนมดบกยวรล])ั', r'\1ั\2', text)
    text = re.sub(r'([ก-ฮ])([งนมดบกยวรล])็', r'\1็\2', text)
    text = re.sub(r'([ก-ฮ])าํ', r'\1ำ', text)

    # 2. แก้ไขไม้โทที่กลายเป็น 2, 3 หรือ #
    text = re.sub(r'([ก-ฮ])ั[23#]([ก-ฮ])', r'\1ั้\2', text)
    text = re.sub(r'ขั[#23]น', 'ขั้น', text)
    text = re.sub(r'นี[23#]', 'นี้', text)
    text = re.sub(r'ปี[2#]', 'ปี้', text)
    text = re.sub(r'เพิ[2#]ม', 'เพิ่ม', text)
    text = re.sub(r'สิ[23#]น', 'สิ้น', text)

    # 3. แก้ไขไม้เอกที่กลายเป็น 2, #, 1, L หรือ l
    text = re.sub(r'([ก-ฮะ-ูเ-ไ])ที[2#]', r'\1ที่', text)
    text = re.sub(r'(^|\s)ที[2#]', r'\1ที่', text)
    text = re.sub(r'ที[L|1l!]', 'ที่', text)
    text = re.sub(r'เพื[L|1l!]อ', 'เพื่อ', text)
    text = re.sub(r'ชื[L|1l!]อ', 'ชื่อ', text)

    # 4. แก้ไขสระหน้า + พยัญชนะ + 2/3 เช่น ได2 -> ได้, ให2 -> ให้
