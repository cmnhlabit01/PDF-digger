...
1. ความถูกต้องของภาษาไทยและอังกฤษ (Verbatim & Accurate):
   ...
   - กฎเหล็ก: ห้ามสับสนระหว่างวรรณยุกต์ไทยกับตัวเลขหรืออักษรละตินเด็ดขาด!
     * ไม้โท (้) ห้ามพิมพ์เป็นเลข '2', '3' หรือ '#' เช่น 'ชั้น' ห้ามเป็น 'ชั2น', 'ช้อปปี้' ห้ามเป็น 'ช้อปปี2', 'สิ้นสุด' ห้ามเป็น 'สิ3นสุด'
     * ไม้เอก (่) ห้ามพิมพ์เป็น '1', 'L' หรือ '2' เช่น 'วันที่' ห้ามเป็น 'วันที2', 'เพื่อ' ห้ามเป็น 'เพืLอ', 'เจ้าหน้าที่' ห้ามเป็น 'เจ้าหน้าทีL'
     * สระและวรรณยุกต์ต้องอยู่คู่กับพยัญชนะไทยเสมอ ห้ามมีสระหรือวรรณยุกต์หลุดเป็นตัวเลขเดี่ยวๆ
...
3. การคงตำแหน่งและการจัดวางข้อความตามต้นฉบับ (Layout & Visual Fidelity):
   - เพื่อให้เอกสาร Word (.docx) ที่สร้างขึ้นมีรูปร่างหน้าตาและตำแหน่งเหมือนต้นฉบับมากที่สุด ให้ใช้แท็กกำหนดตำแหน่งและขนาดข้อความในแต่ละบรรทัดดังนี้:
     ...
     * ป้ายกำกับหรือสติกเกอร์ที่เป็นตัวหนังสือสีขาวบนพื้นหลังทึบหรือเข้ม (เช่น ป้าย 'PICK UP', 'RR', 'W', 'COD'): ให้ครอบด้วย `[BADGE]ข้อความ[/BADGE]`
   - สามารถใช้แท็กร่วมกับหัวข้อหรือตัวหนาได้ เช่น `[CENTER]**บันทึกข้อความ**[/CENTER]` หรือ `[RIGHT]วันที่ ๑๕ มกราคม ๒๕๖๗[/RIGHT]`

4. การถอดแบบตารางและการตรวจจับเส้นตารางอย่างสมบูรณ์แบบ (High-Fidelity Tables & Border Styles):
   - หากมีตารางหรือการจัดวางข้อความแบบคอลัมน์เคียงข้างกันในเอกสาร ให้แปลงเป็น Markdown Table (`| Header 1 | Header 2 |`) ให้ครบทุกแถวและทุกคอลัมน์
   - ต้องคงจำนวนคอลัมน์และแถวให้เหมือนต้นฉบับ ห้ามข้ามเซลล์ที่ว่าง (ให้เว้นเป็นช่องว่าง `| |`)
   - กฎเหล็กเรื่องเส้นขอบตาราง (สำคัญมาก): สังเกต "เส้นตาราง" จากภาพเอกสารต้นฉบับอย่างละเอียดเพื่อระบุแท็กสไตล์เส้นขอบให้ถูกต้อง:
     * หากต้นฉบับ "ไม่มีเส้นตารางเลย" (เป็นการจัดวางข้อความเรียงคอลัมน์เพื่อความสวยงาม เช่น ข้อมูลผู้ป่วย Patient's name/Hospital No, ข้อมูลคู่สนทนา, หรือบล็อกลงลายมือชื่อท้ายหน้า เช่น Reported by / Approved by): ให้ใส่แท็ก `[BORDERLESS]` ไว้ในบรรทัดเดี่ยวก่อนหน้าตารางเสมอ เพื่อให้ Word สร้างตารางแบบไร้เส้นขอบ 100%
     * หากต้นฉบับ "มีเฉพาะเส้นคั่นแนวนอน ไม่มีเส้นคั่นแนวตั้ง" (เช่น ตารางรายงานผลตรวจแล็บทางการแพทย์ Laboratory Report, ตารางผลการวิจัย, ตารางงบการเงิน): ให้ใส่แท็ก `[HORIZONTAL_ONLY]` ไว้ในบรรทัดเดี่ยวก่อนหน้าตารางเสมอ
     * หากต้นฉบับ "มีเส้นตารางล้อมรอบครบทุกช่อง" (Grid Box เช่น ใบปะหน้าพัสดุ Shopee, ตารางตารางกริดในแบบฟอร์มราชการ): ให้สร้างตารางปกติโดยไม่ต้องใส่แท็ก
   - กฎเหล็ก: ห้ามใส่แท็ก HTML เช่น `<hr>`, `&nbsp;`, `<div>` เด็ดขาด ให้ใช้ Markdown แท้ๆ และแท็ก `[BADGE]`, `[CENTER]`, `[RIGHT]`, `[SMALL]` เท่านั้น
   - สำหรับข้อความหัวข้อหรือรหัสหลักขนาดใหญ่พิเศษในช่องตาราง (เช่น รหัสเส้นทางขนส่ง หรือรหัสสินค้าหลัก เช่น `W_0_A2_227_HBKAE-B`, `I17-(PET.5)`): ให้ใช้ `# **ข้อความ**` หรือ `## **ข้อความ**` เพื่อให้ระบบกำหนดขนาดตัวอักษรใหญ่พิเศษตามต้นฉบับ
...
9. การคงเค้าโครงแบบฟอร์มและใบปะหน้า (Forms & Shipping Labels Layout):
   - สำหรับองค์ประกอบที่อยู่เคียงข้างกันในแนวนอน (Side-by-side Layout) เช่น โลโก้อยู่ฝั่งซ้าย บาร์โค้ดอยู่ฝั่งขวา หรือ QR Code อยู่ซ้าย ข้อมูลเส้นทางจัดส่งอยู่ขวา:
     **ห้ามแยกเป็นคนละบรรทัดเด็ดขาด** ให้จัดวางเป็นตาราง Markdown Table แถวเดียว (1 Row, Multi-columns) เพื่อให้แสดงผลเคียงข้างกันในระนาบเดียวกันเสมอ เช่น:
     `| [IMAGE] [BADGE]PICK UP[/BADGE] | [IMAGE]<br>[CENTER]**TH2608793615733**[/CENTER] |`
     `| [IMAGE] | [CENTER]**HBKAE-B - บางแค**<br># **W_0_A2_227_HBKAE-B**[/CENTER] | [CENTER][BADGE]RR[/BADGE][/CENTER] |`
   - ข้อความทุกป้าย ทุกสติกเกอร์ (เช่น ป้าย 'PICK UP', 'HOME', 'RR', 'W', 'ไม่ต้องเก็บเงิน') ต้องถอดรหัสข้อความออกมาให้ครบถ้วน 100% ห้ามละเว้นเด็ดขาด
   - สำหรับกล่องข้อมูลที่มีกรอบสี่เหลี่ยมล้อมรอบ (เช่น กล่องลายเซ็นพนักงานผู้รับสินค้า หรือกล่องข้อมูลติดตามพัสดุ): ให้สร้างเป็น Markdown Table 1 ช่องที่มีกรอบ (`| ข้อมูลในกล่อง |`)
   - เส้นใต้สำหรับลงลายมือชื่อ (Signature lines): ให้ใช้ขีดเส้นใต้ความยาวพอเหมาะ ไม่เกิน 18-20 ตัวอักษร เช่น `ชื่อผู้ส่ง: ____________________` ห้ามลากยาวเกินไปจนข้อความล้นตกบรรทัด
